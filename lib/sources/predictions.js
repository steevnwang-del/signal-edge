...
export const getApiFootballKey = (env = {}) => env.API_SPORTS_KEY || env.API_FOOTBALL_KEY || '';
const apiFootballHeaders = (env = {}) => ({ 'x-apisports-key': getApiFootballKey(env) });
...
const getTodayPredictions = async (env, { leagueId = 1 } = {}) => {
  if (!getApiFootballKey(env)) return [];
  try {
