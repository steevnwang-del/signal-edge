...
const WC_TEAM_IDS = {
  // Group A
  'United States': 2415, 'USA': 2415, 'Panama': 168, 'Canada': 107, 'Jamaica': 119,
  // Group B  
  'Argentina': 26, 'Chile': 34, 'Peru': 41, 'Bolivia': 35,
  // Group C
  'Mexico': 16, 'Uruguay': 38, 'Ecuador': 40, 'Venezuela': 79,
  // Group D
  'Brazil': 6, 'Colombia': 39, 'Paraguay': 37, 'Costa Rica': 100,
  // Group E
  'Germany': 25, 'Portugal': 27, 'Scotland': 1159, 'Georgia': 1218,
  // Group F
  'Spain': 9, 'Netherlands': 1, 'Norway': 152, 'Croatia': 3,
  // Group G
  'France': 2, 'England': 10, 'Algeria': 57, 'Senegal': 67,
  // Group H
  'Morocco': 31, 'Serbia': 14, 'Denmark': 21, 'South Africa': 90,
  // Group I
  'Japan': 47, 'Australia': 97, 'Indonesia': 635, 'Uzbekistan': 1065,
  // Group J
  'South Korea': 48, 'Saudi Arabia': 116, 'Iraq': 1294, 'Jordan': 351,
  // Group K
  'Iran': 55, 'Qatar': 164, 'Kyrgyzstan': 1289, 'North Korea': 1305,
  // Group L
  'Nigeria': 54, 'Ivory Coast': 3224, 'Congo DR': 66, 'Tanzania': 2563,
  // Others
  'Switzerland': 15, 'Poland': 24, 'Belgium': 4, 'Tunisia': 62,
  'Ghana': 96, 'Cameroon': 65, 'Egypt': 52,
};
...
const fetchTeamContext = async (homeTeamEn, awayTeamEn, sport, env) => {
  ...
  try {
    ...
    // If we don't have IDs, still return a note that data was unavailable
    if (!homeId || !awayId) {
      console.warn(`[fetchTeamContext] No ID for ${!homeId ? homeTeamEn : awayTeamEn}`);
      return null;
    }
