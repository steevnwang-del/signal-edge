...
export const classifyDecision = ({ evPct = 0, edgePct = 0, dataCompleteness = 50, risk = 50, bestOdds = 2.0 } = {}) => {
  // 高賠率（>6.0）冷門隊：模型沒有足夠數據超越市場，強制 WAIT
  // EV 高是因為市場給的機率本來就低，+0.7% 修正無法代表真實 edge
  if (bestOdds > 6.0 && dataCompleteness < 75) return 'WAIT';
  // 極高賠率（>10.0）：永遠 WAIT，模型不適用
  if (bestOdds > 10.0) return 'WAIT';
...
export const evaluateMarket = ({ marketRows = [], modelMap = {}, dataCompleteness = 60, risk = 50 } = {}) => {
  ...
  const bestOdds = best?.price || 2.0;
  const decision = best ? classifyDecision({ evPct: best.evPct, edgePct: best.edgePct, dataCompleteness, risk, bestOdds }) : 'WAIT';
