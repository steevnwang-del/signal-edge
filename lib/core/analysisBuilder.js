...
import { buildDecisionEngine } from './decisionEngine.js';
...
export const buildAnalysisFromOddsEvent = (event, {
  ...
  dataBlock.decisionEngine = buildDecisionEngine(dataBlock);
  dataBlock.probabilityScore = dataBlock.decisionEngine.probabilityScore;
  dataBlock.valueScore = dataBlock.decisionEngine.valueScore;
  dataBlock.riskScore = dataBlock.decisionEngine.riskScore;
  dataBlock.beginnerLane = dataBlock.decisionEngine.beginnerLane;
  dataBlock.advancedLane = dataBlock.decisionEngine.advancedLane;
  dataBlock.bettingConditions = dataBlock.decisionEngine.conditions;
  dataBlock.decisionTags = dataBlock.decisionEngine.tags || [];
  dataBlock.qualityTags = [...new Set([...(signalFusion.tags || []), ...(dataBlock.decisionEngine.tags || [])])].slice(0, 10);
...
const compactDataBlockForAI = (db) => ({
  ...
  decisionEngine: db.decisionEngine || null,
  beginnerLane: db.beginnerLane || null,
  advancedLane: db.advancedLane || null,
  bettingConditions: db.bettingConditions || null,
  quality: {
    ...
    probabilityScore: db.probabilityScore,
    valueScore: db.valueScore,
    riskScore: db.riskScore,
    decisionTags: db.decisionTags,
...
- DATA_BLOCK_JSON.contentQuality / signalFusion / decisionEngine 是內容質量、訊號一致性與雙軌決策評估，必須用來說明高機率方向、價值方向與風險。
- decision 已由模型決定為 ${db.decision}，不得更改；${decisionContext}。但可以同時說明『新手高機率方向』與『進階價值判斷』的差異。
...
【高機率方向】
用 70-110 字說明 decisionEngine.probabilityScore、beginnerLane、推薦方向與是否適合新手；必須提醒高機率不等於保證。

【價值下注判斷】
用 80-130 字引用 decisionEngine.valueScore、advancedLane、EV、Edge、fair odds/最低參考賠率；說明報酬是否被市場吃掉。沒有賠率時說明不計算 EV。
...
【下注條件與放棄條件】
用 90-150 字整理 bettingConditions.entryConditions 與 avoidConditions；要分清楚新手小注參考與進階價值下注。

【主要風險與限制】
用 70-100 字說明資料完整度、riskScore、資料缺口與取消條件。
...
export const fallbackNarrative = (a) => {
  ...
  const beginnerLine = `${db.decisionEngine?.beginnerLane?.label || db.beginnerLane?.label || '觀察'}：${db.decisionEngine?.beginnerLane?.note || db.beginnerLane?.note || '依勝率穩定分、資料完整度與風險判斷。'}`;
  const advancedLine = `${db.decisionEngine?.advancedLane?.label || db.advancedLane?.label || '等待價格'}：${db.decisionEngine?.advancedLane?.note || db.advancedLane?.note || '依 EV、Edge、最低參考價與市場是否過熱判斷。'}`;
  const entry = (db.bettingConditions?.entryConditions || []).slice(0, 2).join('；') || '等待賽前資料確認';
  const avoid = (db.bettingConditions?.avoidConditions || db.cancelConditions || []).slice(0, 2).join('；') || '臨場資料反轉或賠率失效';

  if (!hasOdds) {
    return `【一句話結論】等待賠率確認
【資料來源與模型框架】本場目前只有賽程與隊伍資訊，缺少可用 H2H 市場賠率，因此不計算投注 EV、Edge 或最低參考賠率。
【賽事背景與雙方狀態】${db.home} 對 ${db.away}，開賽時間 ${db.timeStr || '未提供'}。${prediction}
【高機率方向】勝率穩定分 ${db.decisionEngine?.probabilityScore || db.probabilityScore || 0}/100；${beginnerLine} 高機率不等於保證結果。
【價值下注判斷】價值分數 ${db.decisionEngine?.valueScore || db.valueScore || 0}/100；因無賠率，不計算 EV 與最低參考價。
...
【國外分析大師雷達】${analysts}
【國際新聞與觀點】${insights}
【下注條件與放棄條件】條件：等待賠率、陣容/版本與官方賽程確認。放棄：賠率長時間缺失或臨場資訊反向。
【主要風險與限制】資料完整度 ${db.dataCompleteness}/100，風險分數 ${db.riskScore || db.risk}/100；主要限制是賠率、陣容、版本與臨場資訊不足。
【賽前確認事項】等待賠率、陣容/版本與官方賽程確認後再更新。
【SignalEdge 判斷】目前只適合情報觀察，不作下注建議。`;
  }

  return `【一句話結論】${db.decisionEngine?.segmentLabel || db.decision}：${db.decisionEngine?.recommendedName || db.pickName}
【資料來源與模型框架】本場使用市場去水、fair odds、模型機率、國外分析大師牆與資料完整度評估。${prediction}
【賽事背景與雙方狀態】${db.home} vs ${db.away}，開賽時間 ${db.timeStr || '未提供'}。${db.homeForm ? `${db.home} 近況 ${db.homeForm}，${db.away} 近況 ${db.awayForm || '未提供'}。` : '近期狀態資料未提供。'}
【高機率方向】勝率穩定分 ${db.decisionEngine?.probabilityScore || db.probabilityScore || 0}/100；${beginnerLine} 高機率不等於保證，偏向命中率需求時仍需控注碼。
【價值下注判斷】價值分數 ${db.decisionEngine?.valueScore || db.valueScore || 0}/100；${advancedLine} EV ${db.ev}%、Edge ${db.edge}%，最低參考價 ${db.minOdds || '—'}。
【模型與市場解讀】模型機率 ${db.modelHome}% / ${db.modelDraw ? db.modelDraw + '% / ' : ''}${db.modelAway}%，市場去水機率 ${db.marketHome}% / ${db.marketDraw ? db.marketDraw + '% / ' : ''}${db.marketAway}%。${scores}
【內容質量評分】${qualityLine}
【國外分析大師牆】${masterWall}${foreignPicks}
【國外分析大師雷達】${analysts}
【國際新聞與觀點】${insights}
【下注條件與放棄條件】條件：${entry}。放棄：${avoid}。
【主要風險與限制】資料完整度 ${db.dataCompleteness}/100，風險分數 ${db.riskScore || db.risk}/100；臨場陣容、先發、版本或盤口變動可能改變判斷。
【賽前確認事項】確認首發/投手/陣容/版本與賠率是否仍高於最低參考價。
【SignalEdge 判斷】高機率方向與價值下注是兩條不同邏輯；此內容是機率與價格比較，不代表結果保證。`;
};
