...
export const buildPromptFromDataBlock = (dataBlock, settings = {}) => {
  ...
  const sportTips = isSoccer
    ? '足球：重點解釋 Elo/Poisson、模型機率、去水市場、比分分布與資料缺口。世界杯可提國際媒體觀點；分析師雷達只能作為應核對來源，不能當成即時立場。'
    : isMLB
...
- DATA_BLOCK_JSON.analystRadar 是國外分析師/數據源雷達，不是即時推薦；除非 internationalInsights 有具體文章或摘錄，不能說某來源看好哪隊。
...
【資料來源與模型框架】
用 70-110 字說明這場分析用了哪些資料：市場去水、fair odds/最低參考賠率、Elo/Poisson、外部預測、國際觀點與分析師雷達。沒有的資料要說缺失。
...
【國外分析師雷達】
用 60-100 字整理 DATA_BLOCK_JSON.analystRadar 中應優先核對的國外分析師/數據源，以及它們應檢查什麼；不得寫成該來源已經推薦某隊。
...
export const fallbackNarrative = (a) => {
  ...
  const analysts = db.analystSignals?.length
    ? `國外分析師雷達：${db.analystSignals.slice(0,3).map(x=>`${x.name || x.source}（${x.category || '觀點源'}）`).join('、')}，只作賽前核對，不代表該來源已給出推薦。`
    : '國外分析師雷達尚未命中此賽事。';
