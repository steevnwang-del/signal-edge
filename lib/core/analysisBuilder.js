...
export const buildPromptFromDataBlock = (dataBlock, settings = {}) => {
  ...
  const sportTips = isSoccer
    ? '足球：重點解釋 Elo/Poisson、模型機率、去水市場、比分分布與資料缺口。世界杯可提國際媒體觀點，但只能引用 DATA_BLOCK 的 internationalInsights。'
    : isMLB
...
【資料來源與模型框架】
用 70-110 字說明這場分析用了哪些資料：市場去水、fair odds/最低參考賠率、Elo/Poisson、外部預測、國際觀點。沒有的資料要說缺失。
...
【國際觀點重點】
用 60-100 字整理 DATA_BLOCK_JSON.internationalInsights 的標題或摘要；沒有就說目前國際觀點快取尚未命中此賽事。
